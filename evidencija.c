...
// 25, 26: Demonstracija qsort (rekurzija + pokazivač na funkciju)
int usporediCijene(const void* a, const void* b) {
    float c1 = *(const float*)a;
    float c2 = *(const float*)b;
    if (c1 < c2) return -1;
    if (c1 > c2) return 1;
    return 0;
}

void sortirajCijeneQsort(const KvarPopravak* glava) {
    float cijene[1000];
    int n = 0;
    while (glava && n < 1000) {
        cijene[n++] = glava->cijena;
        glava = glava->next;
    }
    qsort(cijene, n, sizeof(float), usporediCijene); // 25, 26
